...
export type AnimationType = 'element' | 'text'
 ...
 * @param animationType - 动画类型：element(位置/尺寸) | text(字体大小)
 */
export function play(
  ...
  animationType: AnimationType = 'element'
): void {
  const cache = getCache()
  ...
  if (animationType === 'text') {
    // 文字模式：仅变化字体大小和位置
    const computedStyle = window.getComputedStyle(el)
    const targetFontSize = parseFloat(computedStyle.fontSize) || first.fontSize || 16
    el.style.setProperty("--origin-font-size", (first.fontSize || 16) + 'px');
    el.style.setProperty("--target-font-size", targetFontSize + 'px');
    el.style.setProperty("--tX", dXY.dx + 'px');
    el.style.setProperty("--tY", dXY.dy + 'px');

    // 文字模式动画
    if(isReverse) {
      el.style.animation = `text_hide ${ duration }ms forwards`;
    } else {
      el.style.animation = `text_show ${ duration }ms forwards`
    }
  } else {
    // 元素模式：变化位置和尺寸
